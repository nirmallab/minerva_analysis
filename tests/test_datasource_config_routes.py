...
from the real app data directory via monkeypatching minerva_analysis.data_path/
...
def test_upload_page_renders_source_type_tabs(tmp_path, monkeypatch):
    monkeypatch.setattr(minerva_analysis, "data_path", tmp_path)
    monkeypatch.setattr(minerva_analysis, "config_json_path", tmp_path / "config.json")
    client = minerva_analysis.app.test_client()
