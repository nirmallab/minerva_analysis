...
from minerva_analysis import datasource
...
def test_upload_channels_route_renames_without_header(tmp_path, monkeypatch):
    data_dir = _register(tmp_path)
    monkeypatch.setattr(minerva_analysis, "data_path", data_dir)
    monkeypatch.setattr(minerva_analysis, "config_json_path", data_dir / "config.json")
