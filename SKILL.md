...
- `minerva_analysis/server/models/adapters/`: input-format adapter layer (CSV/AnnData) -- see "Multi-Modal Datasource Support" below.
...
- `minerva_analysis/server/models/database_model.py`: SQLite models (`ChannelList`, `GatingList` -- persisted per-datasource channel/gating UI state, not just image/feature data).
- `minerva_analysis/server/models/centroid_tiles.py`: centroid manifest/tile cache. Dispatches through `get_adapter()` (`_load_table()` helper), not a raw `pl.read_csv()` -- must stay format-agnostic (was AnnData-broken once, see below).
...
- `minerva_analysis/server/routes/import_routes.py`: CSV upload/import flow routes (positional/header-matching wizard, `channelMatch.js`-driven).
- `minerva_analysis/server/routes/datasource_config_routes.py`: AnnData import flow routes (`import_anndata`/`save_datasource_config`) -- a separate, non-positional wizard driven by `inspect_anndata()`'s JSON payload.
...
- `minerva_analysis/client/src/js/views/csvGatingList.js`: gating marker list + sliders. Its `columns` are the *feature-table* columns (e.g. `adata.var_names`, or CSV headers) -- never the image channel list. See "Multi-Modal Datasource Support" below for why that distinction matters.
...
- `minerva_analysis/client/src/js/views/channelList.js`: image *channel* list/rendering picker (opacity, color, threshold) -- driven by `config.imageData`, a different vocabulary than the gating marker list above for AnnData datasources.
- `minerva_analysis/client/templates/datasource_config.html` + `minerva_analysis/client/src/js/views/datasourceConfig.js`: standalone AnnData "configure datasource" page (coordinates/features/subset/transform), separate from the CSV `channelMatch.js` wizard.
...
- `db.sqlite3` (`data_path / "db.sqlite3"`, holds `ChannelList`/`GatingList`) can be silently orphaned: if the file is deleted (e.g. moved to Trash) while a server process still has it open, that process keeps reading/writing the deleted inode indefinitely (basic POSIX behavior) while any *new* process opening the same path gets a fresh, empty file at that path -- the two are now completely disconnected despite sharing a filename. Symptoms: reads succeed and return old data forever, but writes through that same live process can start failing (seen as generic 500s with no useful traceback) once the underlying volume/OS makes the unlinked inode unreliable to write to. Diagnose with `lsof -p <server-pid>` and look for the db file resolving to `~/.Trash/...` or `(deleted)` instead of its real configured path; fix is a genuine process restart (not just editing files), since a new process reopens the real path fresh.
